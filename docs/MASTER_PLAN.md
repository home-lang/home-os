...
| S6 | No HID driver, so still no USB keyboard, and no VFS integration for USB storage. The host-controller driver and the mass-storage read *and write* paths are done: enumeration through Configure Endpoint, Bulk-Only Transport, INQUIRY, READ CAPACITY(10), READ(10) and WRITE(10) — the boot gate reads a known block off a USB disk, writes another, reads it back, and then checks the host image, since a device accepting a write is not the same as the write reaching the media | `kernel/src/drivers/usb.home` | P2 | Phase 7a (keyboards, storage on metal) |
