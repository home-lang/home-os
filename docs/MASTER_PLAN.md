...
| S18 | seccomp does not cover the syscalls `usermode.home` answers itself. `syscall_dispatch` filters every call that reaches the kernel's table, but `read`, `write`, `exit`, `fork`, `wait`, `kill`, `getpid`, `nanosleep`, the signal calls and the pipe paths are all intercepted before that — so a filter forbidding `write` does not stop one. The interception exists because those calls need the caller's frame or address space, which the table does not hold; the filter has to move to the entry point rather than the calls moving back | `kernel/src/arch/x86_64/usermode.home` | P2 | Phase 6 (capability security) |
