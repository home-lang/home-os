...
| S4 | No delete path — nothing a volume holds can be removed from it — and objects carry no owner or mode, so the VFS's permission checks have nothing to check a homefs file against. Steps 1–4 and 6 of `docs/design/homefs.md` §9 are done: on-disk structures with blake2s checksums, a block device, copy-on-write B-trees, the commit protocol, snapshots with rollback, free-space reclamation, VFS routing (a volume mounts at a path and `open`/`read`/`write`/`lseek`/`readdir` reach it), and objects — files, directories and extent trees, all copy-on-write. A volume survives a reboot, `scripts/crash-gate.sh` kills QEMU mid-commit and remounts to prove it, a snapshot survives a reboot too, `homefs_fsck()` walks the active root and every pinned snapshot root read-only, and the boot gate creates a directory, writes a 5000-byte file across an extent boundary, reads it back byte for byte, then corrupts the object table and checks fsck catches it | `kernel/src/fs/homefs.home` | P2 (CoW implementation) | Phase 2 storage; Phase 6 `snapshot-rollback` |
...
| S6 | No HID driver, so still no USB keyboard, and no write path or VFS integration for USB storage. The host-controller driver and a mass-storage read path are done: enumeration through Configure Endpoint, Bulk-Only Transport, INQUIRY, READ CAPACITY(10) and READ(10) — the boot gate reads a known block off a USB disk and checks its contents | `kernel/src/drivers/usb.home` | P2 | Phase 7a (keyboards, storage on metal) |
| S7 | No AML interpreter and no power-state transitions, so there is still no S3 suspend/resume. Table discovery is done: RSDP from both permitted locations, checksums verified, RSDT/XSDT walked, FADT and MADT parsed | `kernel/src/drivers/acpi.home` | P2 | Phase 7a (power, S3 suspend) |
