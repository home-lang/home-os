...
| S6 | The HID keyboard report layout is decoded but never exercised — a usb-kbd takes keystrokes away from the PS/2 controller, so the gate drives a mouse and the keyboard branch is the one part no test reaches. The host-controller driver and the mass-storage read *and write* paths are done: enumeration through Configure Endpoint, Bulk-Only Transport, INQUIRY, READ CAPACITY(10), READ(10) and WRITE(10), plus the HID class layer (boot protocol, interrupt endpoint, report polling) — the boot gate reads a known block off a USB disk, writes another, reads it back, and then checks the host image, since a device accepting a write is not the same as the write reaching the media, it moves an emulated mouse and reads the movement back off the interrupt endpoint, and the disk is registered as `/dev/usb0` so paths reach it through the VFS | `kernel/src/drivers/usb.home` | P2 | Phase 7a (keyboards, storage on metal) |
