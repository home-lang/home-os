...
| S7 | No S3 suspend/resume. The transition is a PM1_CNT write and its sleep type already comes from the DSDT; what is missing is resume — CPU state saved, the FACS waking vector planted, and the way back — and the boot gate cannot exercise a suspend, since suspending ends the run. Table discovery and the AML namespace are done: RSDP from both permitted locations, checksums verified, RSDT/XSDT walked, FADT and MADT parsed, and the DSDT's declarations parsed — so shutdown takes its SLP_TYP from `\_S5_` instead of assuming 5. An AML interpreter runs methods within an integer subset — locals, args, arithmetic, logic, If/Else/While, Return, Store — and refuses anything outside it rather than guessing; 15 of the 100 methods QEMU's DSDT declares fall inside. The boot gate walks that DSDT to the byte, requires the parser to reject a truncated and a corrupted copy, and runs the interpreter against a hand-assembled method with exact expected values | `kernel/src/drivers/acpi.home` | P2 | Phase 7a (power, S3 suspend) |
