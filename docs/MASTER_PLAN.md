...
| S7 | AML methods are not executed, so there is still no S3 suspend/resume — `_PTS` and `_WAK` are methods and their bodies are stepped over. Table discovery and the AML namespace are done: RSDP from both permitted locations, checksums verified, RSDT/XSDT walked, FADT and MADT parsed, and the DSDT's declarations parsed — so shutdown takes its SLP_TYP from `\_S5_` instead of assuming 5. The boot gate walks QEMU's DSDT to the byte and then requires the parser to reject a truncated and a corrupted copy of it | `kernel/src/drivers/acpi.home` | P2 | Phase 7a (power, S3 suspend) |
