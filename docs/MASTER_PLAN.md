...
| S6 | No class drivers and no interrupt or bulk endpoints, so there is still no USB keyboard and no USB storage. The host-controller driver is done: discovery, bring-up, command ring, port reset, Enable Slot, Address Device, and control transfers on endpoint 0 — the boot gate enumerates a device and reads its descriptor | `kernel/src/drivers/usb.home` | P2 | Phase 7a (keyboards, storage on metal) |
