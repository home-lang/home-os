...
| S16 | `sys/signal.home` does the mask bookkeeping, prints that it is delivering a signal, and then `// Would set up the signal frame on the user stack here` — it never enters a handler. `scheduler.home` calls it on every schedule, so the log says signals are being delivered while none are. Real delivery is in `arch/x86_64/usermode.home`, which rewrites the frame the syscall stub returns through; this module's per-pid state is a second, unused model of the same thing | `kernel/src/sys/signal.home` | P2 | superseded — the entry closes when this module is either wired to the real path or removed |
