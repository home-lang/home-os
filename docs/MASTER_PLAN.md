...
| S4 | No commit protocol, no mount path, no free-space reclamation, so nothing persists across a boot. Steps 1–2 of `docs/design/homefs.md` §9 are done: the on-disk structures with blake2s checksums, a block device, and copy-on-write B-trees with executed tree tests | `kernel/src/fs/homefs.home` | P2 (CoW implementation) | Phase 2 storage; Phase 6 `snapshot-rollback` |
