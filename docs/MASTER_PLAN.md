...
- **Exit gates:**
  ...
  - ✅ **Pipes, signals, fork/exec, and job control demonstrated in tests.** `sigtest` installs a handler, raises the signal, and checks that execution resumes where it left off; `forktest` checks that the child's writes do not reach the parent's memory; `schedtest` interleaves parent and child output; the gate backgrounds a job and reads its exit status out of `jobs`. Pipes are real: `pipetest` asserts twenty-three values across a pipe's behaviour, including that buffered bytes survive the writer closing, that a partial read leaves the remainder queued, and that a child's writes are read by the parent. den runs a pipeline's stages at the same time through kernel buffers — `/bin/cat /etc/lines.txt | /bin/sort | /bin/uniq` is three tasks passing bytes, not three files. The staged path remains as a fallback for shapes that cannot be run concurrently, which is bounded by who can wait: a builtin runs inside the shell and cannot yield, so it may feed a pipeline but not sit in the middle of one.
...
- `kernel/src/crypto/bignum.home`
- `kernel/src/crypto/srp.home`
