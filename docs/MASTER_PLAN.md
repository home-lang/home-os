...
| S15 | `context_switch` cannot switch. It saves the stack pointer, writes CR3, sets the stack pointer to the next task's — and then the function epilogue restores the frame pointer's stack two instructions later, discarding it. A context switch written as an ordinary function cannot work: its own return address and locals live on the stack it is trying to leave. It is reachable from the timer interrupt through `scheduler_tick` and is inert today only because nothing enqueues a task, so the run queue never offers a next one. `scheduler.home` counts the switches it thinks it performed | `kernel/src/core/process.home` | P2 | Phase 3 signals and job control; the same work as S13 |
