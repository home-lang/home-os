...
| S4 | Native filesystem is a 33-line import-satisfying stub. The P1 design doc is **done** (`docs/design/homefs.md`); the P2 CoW implementation is not started | `kernel/src/fs/homefs.home` | P2 (CoW implementation) | Phase 2 storage; Phase 6 `snapshot-rollback` |
...
| S6 | No enumeration and no transfers: nothing is placed on the command ring and no event is consumed, so no device is addressed. Discovery and bring-up are done — the xHCI is found by PCI class, its window mapped, its capability registers read, and it is reset and running with its device-context array and both rings armed | `kernel/src/drivers/usb.home` | P2 | Phase 7a (keyboards, storage on metal) |
