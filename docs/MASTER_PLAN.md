...
| S17 | PS/2 keyboard input is read and dropped. `interrupt_dispatch` reads port 0x60 so the controller releases the line and discards the scancode; `keyboard_interrupt_handler`, which would decode it into the driver's buffer, was reached only from a handler in `main.home` that nothing called, and is now unreachable. Nothing reads that buffer either — the console's input arrives over the serial line — so on real hardware a key press does nothing. The boot gate's `keyboard=N` counts interrupts, not characters, which is why this survived a gate that presses keys | `kernel/src/arch/x86_64/interrupts.home` | P2 | Phase 7a (a keyboard on metal) |
